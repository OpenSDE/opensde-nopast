...
# 
# Filename: target/share/install/build_initrd.sh
...
[ "$boot_title" ] || boot_title="OpenSDE Installation"

. $base/target/share/initrd.in
. $base/target/share/boot.in
...
for x in `egrep 'X .* KERNEL .*' $base/config/$config/packages |
  ...
  kernel=${x/_*/}
  moduledir="`grep $libdir/modules  $build_root/var/adm/flists/$kernel |
